...
export class ___ extends Component {
    ...
    list = {
        ...
        color: this.listTextColor
    }
    log = {
        backgroundColor: this.logBackgroundColor,
        color: this.logTextColor
    }

    _list = {
        backgroundColor: this._listBackgroundColor,
        color: this._listTextColor
    }
    _log = {
        backgroundColor: this._logBackgroundColor,
        color: this._logTextColor
    }
    ...
    changeTargetComponent(target) {
        this.setState({targetComponent: target})
    }
        ...
        return <div style={this.styles.listContainer} className="taskList-container">
            ...
            <div>
                <button
                    className="list"
                    style={this.list}
                    onClick={() => this.changeStatus('list')}>list</button>
                <button
                    className="log"
                    style={this.log}
                    onClick={() => this.changeStatus('log')}>logs</button>
            </div>
            <input
                onChange={e => this
                .props
                .taskListUpdater(e.target.value)}/>
        </div>
    }
    ...
    render() {
        // let handle=".draggable" let defaultPosition={x: 0, y: 0} let position={} let
        // grid=[1, 1]
        ...
        // if (this.state.on === 'list')return this.renderListComponent(); if
        // (this.state.on === 'log')return this.renderLogsComponent();
...
let CategoryNavigation = (props) => {
    ...
    let styles = {
        container: {
            ...
            // marginTop:'25px', backgroundColor:'#2F363D',
            width: '100%'
        },
        cell: {
            width: '90%',
    ...
    return (
        <div style={styles.container}>
            {categories.map((category) => {
                if (focusedCategory === category.label) 
                    return <div
                ...
                return <div
                    style={onHover.state && onHover.target == category.label
                    ? styles.cellOnHover
                    : styles.cell}
                    onMouseEnter={() => onHoverHadeler(category.label)}
                    onMouseLeave={() => offHoverHadeler('')}
...
let TaskListSwitcherWrapper = (props) => {
    ...
    return <div>
        <div>
            <button
                onClick={() => {
                changeTargetComponent('search')
            }}>Search</button>
            <button
                onClick={() => {
                changeTargetComponent('register')
            }}>Register</button>
        </div>
